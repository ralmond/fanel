...
test_that("GradedResponse lprob", {
  ...
  test_dat <- data.frame(Y=Y,theta=theta,w=weights)
  expect_equal(em$lprob(test_dat,c(0,-1,1)),
               sum(weights*
                   log(c(probs[1,1],
                         probs[2,1],
                         probs[3,2],
  ...
  expect_equal(em$lprob(test_dat,c(log(.5),-1,1)),
               sum(weights*
                   log(c(probs[1,1],
                         probs[2,1],
                         probs[3,2],
...
test_that("NormalScore lprob", {
  ...
  theta <- -2:2
  weights <- c(1,1,2,3,3)
  test_dat <- data.frame(theta=theta,w=weights,Y=0)
  expect_equal(em$lprob(test_dat,c(0,0)),
               sum(weights*dnorm(theta,log=TRUE)))
  expect_equal(em$lprob(test_dat,c(1,log(.5))),
